...
Every ticket carries exactly one of these five canonical labels in its `status` frontmatter field (see `issue-tracker.md`). They describe *who acts next*, not how hard the work is. Use the literal strings below - tooling and agents match on them exactly.
